...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				input = -1; // 반복문 첫 번재 바퀴에 잘못 입력하면 종료되는 상황 방지
				sc.nextLine(); // 입력 버퍼에 남아있는 잘못 입력된 문자열 제거 -> 무한 반복 방지
			}
		} while(input != 0);
	}
	...
	public void selectDeptEmp() {
		...
		List<Employee> empList = dao.selectEmpId(departmentTitle);
		printAll(empList);
	}
	...
	public void selectJobAvgSalary() {
		...
		Map<String, Integer> empMap = dao.selectJobAvgSalary();
		printMap(empMap);
	}

	public void printMap(Map<String, Integer> empMap) {
		if(empMap.isEmpty()) {
			System.out.println("조회된 사원 정보가 없습니다.");
