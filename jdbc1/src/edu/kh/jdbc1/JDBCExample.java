...
public class JDBCExample {
	public static void main(String[] args) {
		...
		// 2. DBMS(Oracle)
		// 3. Oracle에서 Java 애플리케이션과 연결할 때 사용할 JDBC를 상속 받아 구현한 클래스 모음
		//    (ojdbc11.jar 라이브러리)
		...
		try {
			...
			// 1. DB 연결에 필요한 Oracle JDBC Driver 메모리에 로드 -> 객체로 만들어 두기
			Class.forName("oracle.jdbc.driver.OracleDriver");
			
			// () 안에 작성된 클래스의 객체를 반환
			// 메모리에 객체가 생성되고 JDBC 필요 시 알아서 참조해서 가능
			// 생략해도 자동으로 메모리 로드가 진행(명시적 작성 추천)
			...
			// 3. SQL 작성
			//    JAVA에서 작성되는 SQL은 마지막에 ;(세미콜론)을 찍지 않아야 함
